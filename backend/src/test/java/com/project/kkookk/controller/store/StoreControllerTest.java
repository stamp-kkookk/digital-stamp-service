...
class StoreControllerTest {
    ...
    void createStore_Success() throws Exception {
        // given
        StoreCreateRequest request = new StoreCreateRequest("새 매장", "주소", "123-456", StoreStatus.ACTIVE);
        StoreResponse response = new StoreResponse(STORE_ID, "새 매장", "주소", "123-456", StoreStatus.ACTIVE, LocalDateTime.now(), LocalDateTime.now(), OWNER_ID);
        given(storeService.createStore(anyLong(), any(StoreCreateRequest.class))).willReturn(response);

        // when & then
        mockMvc.perform(post("/api/owner/stores")
    ...
    void createStore_Fail_InvalidRequest() throws Exception {
        // given
        StoreCreateRequest request = new StoreCreateRequest("", "주소", "123-456", StoreStatus.ACTIVE);
    ...
    void getStores_Success() throws Exception {
        // given
        StoreResponse response = new StoreResponse(STORE_ID, "내 매장", "주소", "123-456", StoreStatus.ACTIVE, LocalDateTime.now(), LocalDateTime.now(), OWNER_ID);
    ...
    void updateStore_Success() throws Exception {
        // given
        StoreUpdateRequest request = new StoreUpdateRequest("수정된 매장", "새 주소", "456-789", StoreStatus.INACTIVE);
        StoreResponse response = new StoreResponse(STORE_ID, "수정된 매장", "새 주소", "456-789", StoreStatus.INACTIVE, LocalDateTime.now(), LocalDateTime.now(), OWNER_ID);
        given(storeService.updateStore(anyLong(), anyLong(), any(StoreUpdateRequest.class))).willReturn(response);

        // when & then
        mockMvc.perform(put("/api/owner/stores/{storeId}", STORE_ID)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
    ...
    void updateStore_Fail_InvalidRequest() throws Exception {
        // given
        StoreUpdateRequest request = new StoreUpdateRequest("", "새 주소", "456-789", StoreStatus.INACTIVE);
    ...
    void deleteStore_Fail_NotFound() throws Exception {
        ...
        doThrow(new BusinessException(ErrorCode.STORE_NOT_FOUND))
                .when(storeService).deleteStore(OWNER_ID, STORE_ID);
