...
public class WalletAccessService {
    ...
    public WalletAccessResponse getWalletInfo(String phoneNumber, String userName, Long storeId) {
        CustomerWallet customerWallet = customerWalletRepository.findByPhoneAndName(phoneNumber, userName)
            .orElseThrow(() -> new BusinessException(ErrorCode.WALLET_NOT_FOUND));

        Store store = storeRepository.findById(storeId)
            .orElseThrow(() -> new BusinessException(ErrorCode.STORE_NOT_FOUND));

        Optional<CustomerStampCard> customerStampCardOpt = customerStampCardRepository.findByCustomerWalletAndStore(
            customerWallet, store);
