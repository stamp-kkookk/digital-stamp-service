...
public record MigrationListItemResponse(
        ...
        @Schema(description = "승인된 스탬프 개수 (승인 시에만 존재)", example = "7")
                Integer approvedStampCount,
        @Schema(description = "반려 사유 (반려 시에만 존재)", example = "이미지가 불명확합니다")
                String rejectReason,
        @Schema(description = "요청 생성 시각", example = "2025-01-15T10:30:00") LocalDateTime requestedAt,
