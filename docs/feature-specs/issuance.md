...
스탬프 적립은 KKOOKK의 핵심 플로우로, 고객이 적립을 요청하면 Owner 백오피스에서 승인/거절하는 **Approval-based** 시스템이다.
POS 연동 없이 고객 요청 -> Owner 승인 -> 폴링 완료 방식으로 동작한다.
...
| Controller (Owner Approval) | `backend/src/main/java/com/project/kkookk/issuance/controller/OwnerApprovalController.java` |
| API Interface (Owner Approval) | `backend/src/main/java/com/project/kkookk/issuance/controller/OwnerApprovalApi.java` |
| Service (Customer) | `backend/src/main/java/com/project/kkookk/issuance/service/CustomerIssuanceService.java` |
| Service (Owner Approval) | `backend/src/main/java/com/project/kkookk/issuance/service/OwnerApprovalService.java` |
...
| Owner Approval Hooks | `frontend/src/features/store-management/hooks/useApproval.ts` |
...
| POST | `/api/customer/issuance-requests/{id}/cancel` | `CustomerIssuanceController.cancelIssuanceRequest()` | CUSTOMER | 적립 요청 취소. PENDING만 가능. 비관적 락으로 동시성 제어. |
| GET | `/api/owner/stores/{storeId}/issuance-requests` | `OwnerApprovalController.getPendingRequests()` | OWNER | 매장의 PENDING 상태 적립 요청 목록 조회. 만료된 요청은 자동 필터링. |
| POST | `/api/owner/stores/{storeId}/issuance-requests/{id}/approve` | `OwnerApprovalController.approveRequest()` | OWNER | 적립 승인. 스탬프 +1, StampEvent(ISSUED) 기록, Goal 도달 시 WalletReward 자동 발급. |
| POST | `/api/owner/stores/{storeId}/issuance-requests/{id}/reject` | `OwnerApprovalController.rejectRequest()` | OWNER | 적립 거절. 상태를 REJECTED로 변경. |
...
- **Owner 소유권 검증**: `Store.ownerAccountId == OwnerPrincipal.ownerId` 확인.
...
```
Customer App              Backend                    Owner 백오피스
    |                        |                            |
    |--POST /api/customer/   |                            |
    |  issuance-requests     |                            |
    |  {storeId,             |                            |
...
- PENDING -> APPROVED: `OwnerApprovalService.approveRequest()` -- 스탬프/리워드 처리 후 상태 전이
- PENDING -> REJECTED: `OwnerApprovalService.rejectRequest()` -- 단순 상태 전이
- PENDING -> EXPIRED: `CustomerIssuanceService.getIssuanceRequest()` (lazy) 또는 `OwnerApprovalService.validateRequestCanBeProcessed()` -- 시간 경과 시 자동 전이
- PENDING -> CANCELLED: `CustomerIssuanceService.cancelIssuanceRequest()` -- 고객이 대기 중 취소
...
|-------|------|----------|-------------|
| `id` | Long | No | 요청 ID |
| `status` | IssuanceRequestStatus | No | PENDING / APPROVED / REJECTED / EXPIRED / CANCELLED |
...
### PendingIssuanceRequestItem (Backend -> Owner)
...
### PendingIssuanceRequestListResponse (Backend -> Owner)
...
### IssuanceApprovalResponse (Backend -> Owner)
...
### IssuanceRejectionResponse (Backend -> Owner)
...
| `status` | VARCHAR(20) | NOT NULL, ENUM | PENDING / APPROVED / REJECTED / EXPIRED / CANCELLED |
...
| TTL 만료 후 Owner 승인 시도 | 410 | `ISSUANCE_REQUEST_EXPIRED` | 요청이 만료되었습니다 |
...
| Owner가 다른 매장 요청 접근 | 403 | `STORE_ACCESS_DENIED` | 매장 접근 권한이 없습니다 |
...
// Owner: usePendingIssuanceRequests (useApproval.ts)
